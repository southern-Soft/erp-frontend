...
export default function ShippingPage() {
  ...
  const [filteredShipping, setFilteredShipping] = useState<any[]>([]);
  const [displayedShipping, setDisplayedShipping] = useState<any[]>([]);
  ...
  const loadShipping = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/buyers/shipping`, {
  ...
  useEffect(() => {
    ...
    if (filters.country && filters.country !== "all") {
      result = result.filter((s: any) => s.destination_country === filters.country);
    }
    setFilteredShipping(result);
  }, [shipping, filters]);

  // Apply row limit
  useEffect(() => {
    if (rowLimit === "all") {
      setDisplayedShipping(filteredShipping);
    } else {
      setDisplayedShipping(filteredShipping.slice(0, rowLimit));
    }
  }, [filteredShipping, rowLimit]);

  const clearFilters = () => setFilters({ search: "", buyer: "", country: "" });
  const uniqueCountries = [...new Set(shipping.map((s: any) => s.destination_country).filter(Boolean))].sort();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingShipping
        ? `${API_BASE}/buyers/shipping/${editingShipping.id}`
        : `${API_BASE}/buyers/shipping`;
      ...
      if (!response.ok) {
        throw new Error(`Failed to save shipping: ${response.status}`);
      }
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this shipping info?")) {
      try {
        const API_BASE = process.env.NEXT_PUBLIC_API_URL ? `${process.env.NEXT_PUBLIC_API_URL}/api/v1` : '/api/v1';
        const response = await fetch(`${API_BASE}/buyers/shipping/${id}`, {
          method: "DELETE",
          headers: { 'Content-Type': 'application/json' },
        });
        
        if (!response.ok) {
          throw new Error(`Failed to delete shipping: ${response.status}`);
        }
            ...
            {displayedShipping.length === 0 ? (
              <TableRow>
                <TableCell colSpan={9} className="text-center text-muted-foreground">
                  {shipping.length === 0
                    ? "No shipping info found. Add your first shipping destination to get started."
