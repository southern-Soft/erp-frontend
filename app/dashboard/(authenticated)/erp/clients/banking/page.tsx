...
export default function BankingPage() {
  ...
  const loadBanking = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/buyers/banking`, {
  ...
  useEffect(() => {
    ...
    if (filters.clientType && filters.clientType !== "all") {
      result = result.filter((b: any) => b.client_type === filters.clientType);
    }
    // Buyer filter
    if (filters.buyer && filters.buyer !== "all") {
      result = result.filter((b: any) => b.client_type === "buyer" && b.client_id?.toString() === filters.buyer);
    }
    // Supplier filter
    if (filters.supplier && filters.supplier !== "all") {
      result = result.filter((b: any) => b.client_type === "supplier" && b.client_id?.toString() === filters.supplier);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingBanking
        ? `${API_BASE}/buyers/banking/${editingBanking.id}`
        : `${API_BASE}/buyers/banking`;
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this banking info?")) {
      try {
        const API_BASE = process.env.NEXT_PUBLIC_API_URL ? `${process.env.NEXT_PUBLIC_API_URL}/api/v1` : '/api/v1';
        const response = await fetch(`${API_BASE}/buyers/banking/${id}`, {
          method: "DELETE",
          headers: { 'Content-Type': 'application/json' },
        });
        
        if (!response.ok) {
          throw new Error(`Failed to delete banking: ${response.status}`);
        }
