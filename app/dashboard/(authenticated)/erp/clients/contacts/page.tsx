...
export default function ContactsPage() {
  ...
  const loadContacts = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/buyers/contacts`, {
  ...
  useEffect(() => {
    ...
    if (filters.buyer && filters.buyer !== "all") {
      result = result.filter((contact: any) => contact.buyer_id?.toString() === filters.buyer);
    }

    // Supplier filter
    if (filters.supplier && filters.supplier !== "all") {
      result = result.filter((contact: any) => contact.supplier_id?.toString() === filters.supplier);
    }

    // Country filter
    if (filters.country && filters.country !== "all") {
      result = result.filter((contact: any) => contact.country === filters.country);
    }
  ...
  useEffect(() => {
    if (rowLimit === "all") {
      ...
      setDisplayedContacts(filteredContacts.slice(0, rowLimit));
    }
  ...
  const uniqueCountries = [...new Set(contacts.map((c: any) => c.country).filter(Boolean))].sort();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingContact
        ? `${API_BASE}/buyers/contacts/${editingContact.id}`
        : `${API_BASE}/buyers/contacts/`;
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this contact?")) {
      try {
        const API_BASE = process.env.NEXT_PUBLIC_API_URL ? `${process.env.NEXT_PUBLIC_API_URL}/api/v1` : '/api/v1';
        await fetch(`${API_BASE}/buyers/contacts/${id}`, {
          method: "DELETE",
          headers: { 'Content-Type': 'application/json' },
        });
  ...
  const getCompanyName = (contact: any) => {
    if (contact.buyer_id) {
  ...
  const exportColumns: ExportColumn[] = [
    { key: "contact_person_name", header: "Contact Person Name" },
    {
      key: "company",
      header: "Company",
      transform: (value, row) => getCompanyName(row),
    },
